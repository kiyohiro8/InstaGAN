...
class InstaGAN(object):
    ...
    def __init__(self, params):
        ...
        self.D_Y = Discriminator(input_nc)
...
class Generator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=48, 
        ...
        self.mask_decoder = Decoder(1, n_downsampling, ngf * 3, norm_layer, use_bias, use_deconv=use_deconv)
...
class Discriminator(nn.Module):
    def __init__(self, input_nc=3, ndf=48, n_layers=3, norm_layer=nn.BatchNorm2d, use_bias=False):
    ...
    def forward(self, image_masks):
        ...
        mean = (masks).mean(0).mean(-1).mean(-1)
        if mean.sum() <= 0:
